...
export function TourEditor({ tour, scenes: initialScenes, userRole }: TourEditorProps) {
  ...
  useEffect(() => {
    ...
    async function fetchMarkers() {
      const [wpRes, hsRes] = await Promise.all([
        fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/waypoints`,
        ),
        fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/hotspots`,
        ),
      ]);
  ...
  useEffect(() => {
    ...
    async function fetchFloorPlan() {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/floor-plan`,
      );
  ...
  const handleReorderScenes = useCallback(
    (reordered: EditorScene[]) => {
      ...
      // Background sync 鈥?update sort_order for each scene
  ...
  const handleAddScene = useCallback(async () => {
    const res = await fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleTitleChange = useCallback(
    async (newTitle: string) => {
      ...
      fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handlePublish = useCallback(async () => {
    const res = await fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/publish`,
      { method: "POST" }
    );
    if (res.ok) {
  ...
  const handlePlaceWaypoint = useCallback(
    async (position: Position3D) => {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/waypoints`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handlePlaceHotspot = useCallback(
    async (position: Position3D) => {
      ...
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/hotspots`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const syncWaypointPosition = useDebouncedCallback(
    (waypointId: string, position: Position3D) => {
      if (!activeSceneId) return;
      fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/waypoints/${waypointId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const syncHotspotPosition = useDebouncedCallback(
    (hotspotId: string, position: Position3D) => {
      if (!activeSceneId) return;
      fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${activeSceneId}/hotspots/${hotspotId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ position_3d: position }),
  ...
  const handleCameraReset = useCallback(() => {
    // Camera reset placeholder 鈥?SplatViewer manages its own camera
  }, []);
  ...
  const syncSceneProperty = useDebouncedCallback(
    (sceneId: string, field: string, value: unknown) => {
      fetch((process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001") + `/api/orgs/${tour.org_id}/tours/${tour.id}/scenes/${sceneId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
